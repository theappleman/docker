...
class AWS_EC2:
    ...
    def get_tags(self):
        return [{i.instances[0].id:i.instances[0].tags['Name']}
...
ec2 = AWS_EC2(AWS_ACCESS_KEY, AWS_SECRET_KEY)

# Stop processing if AWS_TAG exists on EC2
#if AWS_TAG in ec2.get_tags().keys():
#    print 'Instance: {} already deployed. Not further processing.'.\
#        format(AWS_TAG
#    exit(1)

AWS_TAG = 'staging-{}-docker-ci'.format(len(ec2.get_tags())+1)
...
with cd(DOCKER_PATH):
    ...
    sudo('cd hack/infrastructure/docker-ci/nightlyrelease;'
